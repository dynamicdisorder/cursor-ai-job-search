...
You are orchestrating a two-agent job application workflow. The job posting is provided below as `the user request` (either a URL or pasted text).
...
- If `the user request` looks like a URL, use `WebFetch` to retrieve the job posting content.
...
- Any mention of agentic coding or AI tooling must reference **Cursor** by name
...
- [ ] **Reading order matches the visual order** — section headings appear in the same sequence as on the page, and lines from different sections are not interleaved. The stock banking template is single-column and safe; custom templates registered via `add-template workflow` with sidebars or multi-column layouts are where this breaks.
...
Also mention: once they have actually submitted the application, `outcome workflow <company>` logs it in the tracker and starts the per-application record that `setup workflow` later uses to calibrate the fit framework.
