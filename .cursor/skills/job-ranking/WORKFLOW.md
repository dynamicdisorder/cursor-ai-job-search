...
You are batch-scoring the jobs that `scrape workflow` has collected, so the user can decide where to spend `apply workflow` effort. `scrape workflow` finds and dedupes postings; `apply workflow` evaluates one at a time in depth. `rank workflow` is the bridge: it scores every new posting against the fit framework and returns a ranked shortlist.

`rank workflow` produces **triage scores**, not final evaluations. It scores from the posting text and the candidate profile only - no company research, no reviewer agent. `apply workflow`'s Step 1 evaluation (which adds company research) remains authoritative and always re-runs when the user applies.
...
- A focus area (e.g. `rank workflow data science`) → rank only jobs whose title or stored fit-notes match the focus
...
1. Read `job_scraper/seen_jobs.json`. If the file is missing or has no entries, tell the user to run `scrape workflow` first and stop.
...
4. If no candidates remain, say so ("Nothing new to rank - run scrape workflow to find fresh postings") and stop.
...
Dispatch parallel `general-purpose` agents via the **Subagent tool**, ~5 jobs per agent (a single agent is fine for ≤5 jobs). Token-efficiency rules, consistent with `apply workflow`:
...
- Subagents fetch each posting URL with WebFetch and score **only from actually fetched content**. If a URL is dead, redirects to a listing page, or the posting has expired, the agent marks that job `expired` - it never scores from the title alone and never fabricates posting content.
- Scope is triage: posting text vs. rubric. **No company research, no salary lookup, no web searches** - that depth belongs to `apply workflow`.
...
Do not modify `job_search_tracker.csv` - that file records applications, and `rank workflow` never applies. Re-running `rank workflow` is idempotent: already-`ranked` jobs are skipped unless `--all` re-scores them.
...
- Say explicitly that these are **triage scores from the posting text only**, and that `apply workflow` will re-evaluate with company research before anything is drafted.
- Then ask: "Want to apply to any of these? Give me the number(s) and I'll start with the full `apply workflow` workflow."
- If the user picks one, run the `apply workflow` workflow on that job's URL, passing the triage verdict as prior context but **re-running the full Step 1 evaluation** - triage never substitutes for it.
...
2. **Triage depth only.** No company research, no salary lookups, no reviewer agents - `rank workflow` exists to be cheap enough to run on every scrape batch.
...
5. **State stays consistent.** `seen_jobs.json` fields are only added, never restructured, so `scrape workflow`'s dedup keeps working; the tracker is read-only for this command.
