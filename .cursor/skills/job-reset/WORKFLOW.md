...
You are resetting parts of the job search framework back to a blank state so the user can start fresh with `setup workflow`.
...
>
> - **`profile`** — Clears candidate data from the skill files (profile, behavioral, STAR examples, profile statements). The framework structure and writing rules are preserved. Use this to re-run `setup workflow` from scratch.
>
...
- `.cursor/skills/job-application/references/05-cv-templates.md` *(profile statements section only — framework structure is preserved)*
- `.cursor/skills/job-application/references/07-interview-prep.md` *(STAR examples and STAR candidates sections only — framework structure is preserved)*
...
<!-- Run setup workflow to populate this file -->
...
<!-- Run setup workflow to populate role-specific profile statements -->
```
...
- Any `## STAR Candidates (Complete Manually)` section added by `setup workflow` Path A
...
<!-- Run setup workflow to populate STAR examples from your actual experience -->
```
...
For each non-empty document subfolder, delete all files within it using Shell `rm`. Do not delete the folder itself, and do not delete `documents/README.md`.
...
> Your candidate profile is now blank. Run `setup workflow` to repopulate it. The command auto-detects any files in your `documents/` folder and offers to read from there; otherwise it walks you through a CV import or interactive interview.

**If documents were reset:**
> The `documents/` folder is now empty. Add your career documents and run `setup workflow` to populate your profile. See `documents/README.md` for instructions on what to put where.

**If both were reset:**
> Both your profile files and documents folder are now empty. Add documents to `documents/` (or skip and use the CV import / interview path), then run `setup workflow`.
