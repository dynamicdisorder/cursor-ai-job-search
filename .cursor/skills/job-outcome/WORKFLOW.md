...
- `job_search_tracker.csv` - the status column that `scrape workflow` and `rank workflow` use for dedup and exclusion
- `documents/applications/<company>_<role>/` - the per-application archive (posting, submitted drafts, `outcome.md`) that `setup workflow` Path A mines to calibrate `04-job-evaluation.md` and surface STAR candidates

`outcome workflow` writes the data; `setup workflow` interprets it. This command never edits the evaluation framework or profile files itself.
...
- A company name (optionally with a role), e.g. `outcome workflow acme` or `outcome workflow acme ml engineer` → target that application
...
**Resolutions** (application closed) - these map to the status enum in `documents/README.md` that `setup workflow` parses:
...
- What they'd do differently, and any signal about what the company valued (these feed `setup workflow`'s calibration and STAR-candidate mining, so concrete beats polished)
...
3. **`outcome.md`** - write or update it in exactly the format documented in `documents/README.md`, so `setup workflow` Path A parses it without special cases:
...
- If 3 or more are resolved (or 2+ share a pattern - same role type rejected twice, same sector going silent), suggest:
  > "You now have <N> resolved applications on record. Run `setup workflow` (Path A) to fold them into your evaluation framework - it calibrates fit scoring from what actually got interviews, and mines your interview feedback for STAR examples."
- Do **not** write anything into `04-job-evaluation.md` or other skill files yourself. `setup workflow` Path A owns that merge - it is read-before-write and idempotent, and duplicating its logic here would race it.
...
>
> - `documents/applications/<company>_<role>outcome workflow.md` - status: <status>, <what changed>
...
1. **Write data, don't interpret it.** The archive and tracker are the outputs; calibration belongs to `setup workflow`. This command never edits profile or framework files.
