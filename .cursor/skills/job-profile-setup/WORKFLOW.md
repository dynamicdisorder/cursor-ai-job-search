...
You are running the onboarding setup for the AI Job Search framework. Your goal is to collect the user's professional information and populate all profile files so the `apply workflow` workflow works out of the box.
...
>
> I'll help you build your professional profile so Cursor Agent can evaluate job postings, tailor CVs, write cover letters, and prepare you for interviews.
>
...
- `.cursor/skills/job-application/references/03-writing-style.md`
- `.cursor/skills/job-application/references/04-job-evaluation.md`
- `.cursor/skills/job-application/references/05-cv-templates.md`
- `.cursor/skills/job-application/references/06-cover-letter-templates.md`
...
Documents cover skills, experience, education, references, and behavioral signal. They do not cover everything `apply workflow` and `scrape workflow` need. After the writes, ask follow-up questions for gaps:
...
Then proceed to Step 3 to populate the non-skill files (`PROFILE.md`, `cv/main_example.tex`, `.cursor/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
This section generates the search queries that power `scrape workflow`. Use the information from Sections 1, 4, and 7 to build targeted queries.
...
### 8. Generate `.cursor/skills/job-scraper/search-queries.md`
...
> - `.cursor/skills/job-application/references/01-candidate-profile.md` - Structured profile
> - `.cursor/skills/job-application/references/02-behavioral-profile.md` - Behavioral assessment
> - `.cursor/skills/job-application/references/04-job-evaluation.md` - Personalized evaluation framework
> - `.cursor/skills/job-application/references/05-cv-templates.md` - CV templates with your profile statements
> - `.cursor/skills/job-application/references/07-interview-prep.md` - STAR examples from your experience
> - `cv/main_example.tex` - Your LaTeX CV template
> - `.cursor/skills/job-scraper/search-queries.md` - Job search queries for `scrape workflow`
>
> **Try it out:**
> - Run `scrape workflow` to search for matching jobs right now
> - Run `apply workflow` with a job posting URL to see the full application workflow
> - Run `setup workflow --section search` later to update your search queries as your priorities evolve
...
- Can be re-run with `--section <name>` to update specific sections (e.g., `setup workflow --section search` to reconfigure job search queries without re-doing the full profile).
...
- At the end, suggest running `scrape workflow` and `apply workflow` with a test job posting.
