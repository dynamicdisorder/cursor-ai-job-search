...
Do reconnaissance before writing any code. Use WebFetch (or `curl` via Shell) on the portal:
...
These conventions are what make portal skills interchangeable for `scrape workflow` and for users reading any skill's docs:
...
- **`SKILL.md` frontmatter:** `name` and a `description` written for Cursor skill triggering - it must name the portal, the market, and include trigger phrases in English **and** the market's language. Document CLI commands in the body instead of using legacy tool allowlists.
...
1. Ask whether the user wants the new portal added to their `scrape workflow` search strategy. If yes, add the portal's site to the relevant query categories in `.cursor/skills/job-scraper/search-queries.md` (site-specific queries, like the existing `jobindex.dk` entries) so `scrape workflow` includes it.
