---
name: job-upskill
description: Compare tracked jobs or a target posting against the candidate profile to identify skill gaps and produce a web-sourced learning plan. Use when the user asks about upskilling, skill gaps, learning plans, or what to learn next.
---
...
`upskill workflow` analyses jobs you have tracked and your current profile to identify skill gaps, then produces a heatmap of those gaps and a learning plan with concrete, web-searched study resources and a recommended study order.

## Invocation

- **`upskill workflow`** — aggregate mode: analyses all jobs in `job_search_tracker.csv`
- **`upskill workflow <URL>`** — targeted mode: analyses a single job posting fetched from the URL
...
- If the invocation was `upskill workflow` with no argument → **aggregate mode**
- If the invocation was `upskill workflow <URL>` → **targeted mode**, store the URL for Step 2
...
3. Read `.cursor/skills/job-application/references/01-candidate-profile.md` to get the candidate's current skills and experience.
...
3. Read `.cursor/skills/job-application/references/01-candidate-profile.md` for the candidate's current skills.
