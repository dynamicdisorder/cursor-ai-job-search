...
The salary lookup tool (`salary_lookup.py`) lets you benchmark company salaries against a baseline from your own data. It's used during the application workflow to show how a company's compensation compares to market rates.

**This tool is optional.** If you don't have salary data, the salary step is simply skipped during `apply workflow`.
...
- If the data file is missing, `salary_lookup.py` exits with a helpful error message and the `apply workflow` workflow skips the salary benchmark step.
