...
**Naming:** Any filename works. If multiple files are present, `setup workflow` reads all of them and cross-references for consistency.

**Tip:** Keep your most comprehensive CV here (not a tailored variant). The skill files are the canonical source — tailored CVs are generated per application by `apply workflow`.
...
**Naming:** Any filename works. Only one LinkedIn export is expected; if multiple are present, `setup workflow` uses the most recently modified one.
...
You can maintain these folders by hand, or let the **`outcome workflow`** command do it: it records progress updates and final results conversationally, archives the submitted drafts and the posting text, keeps `outcome.md` in the format below, and updates `job_search_tracker.csv` in the same step.
...
**`job_posting.md`** — Paste the full job posting text here. Used by `setup workflow` to infer which skills and role types you have targeted, and to calibrate `04-job-evaluation.md`.
...
`in_progress` marks an application that is still open (used by `outcome workflow` for interview-stage updates before a resolution). `setup workflow`'s calibration draws conclusions only from applications with a final status.

**What `setup workflow` learns from outcome.md:**
...
| Format | Readable by `setup workflow` | Notes |
|--------|--------------------------|-------|
